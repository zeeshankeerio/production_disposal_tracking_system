  ...
  const submitCart = async () => {
    ...
    try {
      ...
      for (const entry of cartEntries) {
        ...
        const submissionEntry: Omit<ProductionEntry, 'date' | 'expiration_date'> & { date: string, expiration_date: string } = {
          ...entry,
          ...commonFields,
          product_id: selectedProductData.id,
          date: prepareDateForSubmission(commonFields.date),
          expiration_date: prepareDateForSubmission(entry.expiration_date),
        }
        
        await addProductionEntry(submissionEntry)
      }
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {entry.quantity} units • Expires: {format(entry.expiration_date, "MMM dd, yyyy")}
                    </p>
                    {entry.notes && (
                      <p className="text-sm text-muted-foreground">
                        Notes: {entry.notes}
